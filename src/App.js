...
export default function ExcelProcessor() {
  ...
  const processData = async () => {
    ...
    reordered.forEach(row => {
      if (row.__HEADER__) {
        if (!isFirstBlock) {
          const newTitleRow = worksheet.addRow([titleText]);
          newTitleRow.height = 25;
          worksheet.mergeCells(newTitleRow.number, 1, newTitleRow.number, headersArr.length);
          styleRow(newTitleRow, "FFFFA500");

          const newSupplyRow = worksheet.addRow([supplyText]);
          newSupplyRow.height = 20;
          worksheet.mergeCells(newSupplyRow.number, 1, newSupplyRow.number, headersArr.length);
          styleRow(newSupplyRow, "FFFFFF00");

          const newHeaderRow = worksheet.addRow(headersArr);
          newHeaderRow.height = 20;
          styleRow(newHeaderRow, "FFD3D3D3");
        }
        isFirstBlock = false;
        ...
        const hasFill = !!(dataRow.getCell(1).fill && dataRow.getCell(1).fill.fgColor?.argb);
        if (!hasFill) {
          if (rowCounter % 2 === 0) {
